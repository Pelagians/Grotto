...
DEFAULT_DOCTOR = pathlib.Path(__file__).resolve().parents[1] / "runtimes/chatgpt-desktop/root/usr/local/bin/grotto-doctor"
DOCTOR_PATH = pathlib.Path(os.environ.get("GROTTO_DOCTOR_UNDER_TEST", str(DEFAULT_DOCTOR)))
...
def check(ok: bool = True, stderr: str = "") -> dict[str, object]:
    return {"ok": ok, "returncode": 0 if ok else 1, "stdout": "", "stderr": stderr}
...
class DoctorTest(unittest.TestCase):
    def static_environment(self, stack: ExitStack) -> list[list[str]]:
        ...
        stack.enter_context(mock.patch.object(doctor.shutil, "which", side_effect=lambda name, path=None: "/fake/bwrap" if name == "bwrap" else None))
        stack.enter_context(mock.patch.object(doctor, "effective_landlock_enabled", return_value=(False, check())))
        stack.enter_context(mock.patch.object(doctor, "version_info", side_effect=lambda path, args: {"path": path, "version": None}))
        ...
        stack.enter_context(mock.patch.object(doctor, "network_status", return_value={"host": doctor.AUTH_HOST, "dns": {"ok": None}, "https": {"ok": None}}))
        stack.enter_context(mock.patch.object(doctor, "probe_cache_path", return_value=pathlib.Path("/tmp/cache.json")))
        ...
        stack.enter_context(mock.patch.object(doctor, "read_chatgpt_desktop_security_policy", return_value=security_policy_record()))
        return commands
    ...
    def test_default_collect_does_not_invoke_bwrap(self) -> None:
        ...
        self.assertEqual(report["node_repl_policy_source"], "installed-electron-bundle")
        ...
        self.assertFalse(any(pathlib.Path(command[0]).name == "bwrap" for command in commands))

    def test_json_changes_only_output_formatting(self) -> None:
        report = {"ok": None, "active_probe": False, "sandbox_probe": {"status": "not_run", "reason": doctor.PROBE_NOT_RUN_REASON}}
        stdout = io.StringIO()
        stderr = io.StringIO()
        with mock.patch.object(doctor, "collect", return_value=report) as collect, contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
    ...
    def test_only_probe_option_invokes_active_probe_once(self) -> None:
        with ExitStack() as stack:
            self.static_environment(stack)
            probe = stack.enter_context(mock.patch.object(doctor, "run_sandbox_probe", return_value=probe_record()))
    ...
    def test_human_and_json_rendering_reuse_one_probe_result(self) -> None:
        ...
        self.assertIn("node_repl: exposed=True auto_approved=False verified=True", human)
        self.assertIn("browser_use: trusted_client_hash_patch=True verified=True", human)

    def test_explicit_probe_prints_avc_warning(self) -> None:
        stdout = io.StringIO()
        stderr = io.StringIO()
        with mock.patch.object(doctor, "collect", return_value=probe_record()) as collect, contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            returncode = doctor.main(["--probe-sandbox", "--json"])
        self.assertEqual(returncode, 0)
        collect.assert_called_once_with(active_probe=True)
        self.assertIn("will intentionally trigger host SELinux AVCs", stderr.getvalue())
    ...
    def test_missing_manifest_is_unknown_and_unverified(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            result = doctor.read_chatgpt_desktop_security_policy(pathlib.Path(directory) / "missing.json")
    ...
    def test_valid_manifest_is_collected_without_assuming_exposure(self) -> None:
        manifest = {
            ...
            "node_repl": {"exposed": False, "auto_approved": False, "verified": True, "verification_source": "installed-electron-bundle"},
            "browser_use": {"trusted_client_hash_patch": False, "verified": True},
        }
        with tempfile.TemporaryDirectory() as directory:
            path = pathlib.Path(directory) / "security.json"
            path.write_text(json.dumps(manifest), encoding="utf-8")
            result = doctor.read_chatgpt_desktop_security_policy(path, expected_wrapper_revision=manifest["wrapper_revision"])
    ...
    def test_wrapper_mismatch_is_unverified(self) -> None:
        manifest = {
            "schema_version": 1,
            "wrapper_revision": "7d4049b68b17bc663b8a934326fefcaca99e8ceb",
            "node_repl": {"exposed": True, "auto_approved": False, "verified": True, "verification_source": "installed-electron-bundle"},
            "browser_use": {"trusted_client_hash_patch": True, "verified": True},
        }
        with tempfile.TemporaryDirectory() as directory:
            path = pathlib.Path(directory) / "security.json"
            path.write_text(json.dumps(manifest), encoding="utf-8")
            result = doctor.read_chatgpt_desktop_security_policy(path, expected_wrapper_revision="wrong")
        self.assertFalse(result["node_repl_verified"])
        self.assertFalse(result["browser_use_policy_verified"])
